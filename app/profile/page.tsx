  ...
  return (
    <AppShell>
      <div className="text-center space-y-3 mb-6">
        ...
        <div className="max-w-xs mx-auto space-y-1">
          ...
          <div className="h-2 w-full rounded-full bg-black/10 overflow-hidden">
            <div className="h-full rounded-full bg-btn-green-dark" style={{ width: `${xpProgress}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl bg-answer4 border-2 border-black/10 p-4 flex items-center gap-4 mb-6">
        ...
        <div>
          <div className="font-black text-black">{stats.streak} Day Streak</div>
          <div className="text-xs font-bold text-black/60">Keep playing daily to maintain it!</div>
        </div>
      </div>

      <div className="grid grid-cols-2 gap-3 mb-6">
        {statCards.map(({ label, value, icon: Icon }) => (
          <div key={label} className="rounded-xl bg-white border border-black/15 p-4 flex flex-col items-center gap-1 text-center shadow-soft-1">
